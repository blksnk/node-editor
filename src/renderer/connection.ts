...
const svgElement = <T extends SVGElement>(tagName: string) => document.createElementNS('http://www.w3.org/2000/svg', tagName) as T;
...
const gradientId = (inputColor: GradientColor, outputColor: GradientColor) => `gradient__${ inputColor }__${ outputColor }`

const createConnectionGradient = (inputColor: GradientColor, outputColor: GradientColor) => {
...
export const getUniqueConnections = (nodes: NodeWithId<IOType, IOTypeName>[]): NodeConnection[] => {
  return nodes.map((node) =>
    node.inputs
      .filter(({ connection }) => connection !== undefined && !!connection && !!(connection?.node))
      .map(
        (input) => {
          const outputNode = findById(nodes, input.connection?.node.id as number);
          if (!outputNode) return undefined;

          const outputNodeType = findById(outputNode.outputs, input.connection?.ioId as number)?.type;
...
export const assignIoPositions = (connections: NodeConnection[], rendererNodes: RendererNode<DefinedIOType, IOTypeName>[]): RendererConnection[] => {
  return connections.map((c) => {
    ...
    const outputPosition = getIOIndicatorPosition(outputNode.io.outputs[c.outputNode.ioId])
    const inputPosition = getIOIndicatorPosition(inputNode.io.inputs[c.inputNode.ioId])
    return {
      inputNode: {
...
export const getIOIndicatorPosition = (ioRow: HTMLLIElement): Vec2 => {
  const indicator = ioRow.querySelector('.node__io__indicator') as HTMLDivElement;
...
export const svgBezierCommand = (smooth = 0.5) => (point: Vec2, index: number, points: Vec2[]) => {
...
export const renderConnections = (connections: RendererConnection[], svg: SVGSVGElement) => {

  const paths = connections.map(c => svgPath([ c.outputNode.position, c.inputNode.position ], svgBezierCommand(0.35), getConnectionGradientId(c)))
...
export const renderPendingConnection = (startPos: Vec2, endPos: Vec2, color: GradientColor, svg: SVGSVGElement) => {
  const pendingConnectionGroup = svg.querySelector('#node__connection__pending')
  const path = svgPath([ startPos, endPos ], svgBezierCommand(0.35), gradientId(color, color))
